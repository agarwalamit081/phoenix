...
def _db_tests_enabled() -> bool:
    """Return True when DB-backed test fixtures should run."""
    return os.getenv("RUN_DB_TESTS", "0").lower() in {"1", "true", "yes"}


@lru_cache(maxsize=1)
def _db_available() -> bool:
    """Return True when the configured test database is reachable."""
...
def _db_tests_active() -> bool:
    """Return True when DB tests should execute in this environment."""
    return _db_tests_enabled() and _db_available()


def _truncate_all_tables_sync(conn: Any) -> None:
    """Truncate all public tables in PostgreSQL, preserving schema."""
    rows = conn.execute(
        text(
            """
            SELECT tablename
...
async def _truncate_all_tables_async(conn: Any) -> None:
    """Async variant of public-table truncation helper."""
...
def pytest_collection_modifyitems(config, items):
    """Skip DB-marked test groups when DB tests are disabled or DB is unavailable."""
    if _db_tests_active():
        return

    reason = "DB-backed tests skipped (set RUN_DB_TESTS=1 and provide reachable test DB)"
...
async def test_engine() -> AsyncGenerator[Any, None]:
    ...
    """
    if not _db_tests_enabled():
        pytest.skip("DB-backed fixtures disabled (set RUN_DB_TESTS=1 to enable)")
    if not _db_available():
        pytest.skip("DB-backed fixtures enabled but test DB is unreachable")
...
def cleanup_database_sync() -> Generator[None, None, None]:
    ...
    with cleanup_engine.begin() as conn:
        _truncate_all_tables_sync(conn)
...
async def cleanup_after_test() -> AsyncGenerator[None, None]:
    ...
    # Clean up after the test - truncate all tables in one pass.
    try:
        async with cleanup_engine.begin() as conn:
            await _truncate_all_tables_async(conn)
    except Exception:
        pass
...
def test_client(test_session_maker: Any):
    ...
    # Reset the global engine so tests do not reuse a different event loop's connection pool.
    reset_engine()
...
async def override_db_session(test_session: AsyncSession) -> AsyncGenerator[None, None]:
    ...
    fastapi_app.dependency_overrides[get_db_session] = get_test_session
