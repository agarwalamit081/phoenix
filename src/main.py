...
from src.api.v1 import auth, chat, preferences, routes, voice
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Initialize database (optional — voice endpoints work without it)
    try:
        await init_database()
        logger.info("Database initialized successfully")
    except Exception as e:
        logger.warning(
            f"Database unavailable (PostgreSQL not running?): {e}. "
            "Voice/WebSocket endpoints will still work. "
            "Auth/preferences/routes require the database."
        )
