...
async def bulk_create_preferences(
    ...
    created = await pref_service.bulk_create_preferences(user.id, preferences_data)
    return {
        "created": len(created),
        "preferences": [PreferenceResponse.model_validate(p).model_dump() for p in created],
    }
...
@router.get("/summary/conflicts")
async def get_conflicts_summary(
    user: CurrentUser,
    session: DBSession,
    unresolved_only: bool = Query(True, description="Only show unresolved conflicts"),
) -> dict[str, Any]:
    """Backward-compatible conflicts summary endpoint."""
    pref_service = PreferenceService(session)
    conflicts = await pref_service.get_conflicts(user.id, unresolved_only=unresolved_only)
    return {
        "conflicts": [PreferenceConflictResponse(**c).model_dump() for c in conflicts],
        "count": len(conflicts),
    }
