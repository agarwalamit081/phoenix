...
class LiveKitService:
    ...
    async def _get_room_service(self) -> RoomService:
        """Get or create room service bound to the managed HTTP session.
    ...
    async def _list_rooms_response(self) -> Any:
        """Call LiveKit list rooms API and return raw response."""
        from livekit.api.room_service import ListRoomsRequest

        room_service = await self._get_room_service()
        return await room_service.list_rooms(ListRoomsRequest())

    async def _delete_room(self, room_name: str) -> Any:
        """Call LiveKit delete room API for a room name."""
        from livekit.api.room_service import DeleteRoomRequest

        room_service = await self._get_room_service()
        return await room_service.delete_room(DeleteRoomRequest(room=room_name))

    async def _remove_participant(self, room_name: str, participant_id: str) -> Any:
        """Call LiveKit remove participant API for a room participant identity."""
        from livekit.api.room_service import RoomParticipantIdentity

        room_service = await self._get_room_service()
        return await room_service.remove_participant(
    ...
    async def get_room(self, room_name: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            rooms_response = await self._list_rooms_response()
    ...
    async def remove_participant(
        ...
        try:
            await self._remove_participant(room_name, participant_id)
