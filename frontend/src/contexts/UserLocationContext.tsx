...
const CITY_BOUNDS: Record<string, { bounds: LatLngBounds; timezone: string }> = {
  ...
  "Hyderabad": {
    bounds: { minLat: 17.30, maxLat: 17.50, minLng: 78.35, maxLng: 78.55 },
...
export function UserLocationProvider({
  ...
  const [userLocation, setUserLocation] = useState<UserLocation | null>(() => {
    // Initialize with optional default city
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // Keep null when location access is denied.
      },
      {
  ...
  const updateCity = (city: string, country?: string) => {
    ...
    void (async () => {
      try {
        ...
        // Ignore geocoding failures in client-side fallback.
      }

      const currentLat = userLocation?.latitude ?? 0;
      const currentLng = userLocation?.longitude ?? 0;
      setUserLocation({
        city,
        country: country || userLocation?.country || "",
        latitude: currentLat,
        longitude: currentLng,
        bounds: userLocation?.bounds || calculateBoundsFromCenter(currentLat, currentLng),
